...
pub fn route_request(
    ...
    match (method, path) {
        ...
        (&Method::Post, p) if p.starts_with("/api/blog/update/") => update_blog_post(conn, request),
    ...
    match blog_post {
        Ok(blog) => {
            ...
            if db::get_post_by_slug(&conn.lock().unwrap(), &blog.slug)?.is_some() {
                return Ok(json!({"msg": format!("Blog with slug {} already exists.", blog.slug)}));
            }
            db::create_post(&conn.lock().unwrap(), blog)?;
            Ok(json!({"msg" : "New Blog Created Successfully"}))
        }
        Err(e) => { Ok(json!({"msg" : format!("Failed to Create blog {}", e)})) }
    }
}
    ...
    match blog_post {
        Ok(blog) => {
            ...
            if db::get_post_by_slug(&conn.lock().unwrap(), &blog.slug)?.is_some() {
                ...
                return Ok(json!({"msg" : "Updated your Blog Successfully"}));
            }
            Ok(json!({"msg": format!("No blog found with slug - {}", blog.slug)}))
        }
        Err(e) => { Ok(json!({"msg" : format!("Failed to Update blog {}", e)})) }
    }
}
